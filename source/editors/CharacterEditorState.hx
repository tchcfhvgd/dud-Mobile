...
class CharacterEditorState extends MusicBeatState
{
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			var backgrounddud:FlxSprite = new FlxSprite(-FlxG.width, -FlxG.height).makeGraphic(FlxG.width * 3, FlxG.height * 3, 0xFF888888);
			backgrounddud.scrollFactor.set();
			bgLayer.add(backgrounddud);
			changeBGbutton.text = "white bg";
		} else {
			var backgrounddud:FlxSprite = new FlxSprite(-FlxG.width, -FlxG.height).makeGraphic(FlxG.width * 3, FlxG.height * 3, 0xFFFFFFFF);
			backgrounddud.scrollFactor.set();
			bgLayer.add(backgrounddud);
			changeBGbutton.text = "gray bg";
		}
	}
