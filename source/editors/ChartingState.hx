...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var clear_hurt_notes:FlxButton = new FlxButton(320, clear_notes.y + 30, 'Clear hurt notes', function()
			{
				openSubState(new Prompt('This action will clear current progress.\n\nProceed?', 0, function(){for (sec in 0..._song.notes.length) {
					for(i in 0..._song.notes[sec].sectionNotes.length) {
						if(_song.notes[sec].sectionNotes[i][3] != null) {
							_song.notes[sec].sectionNotes[i] = [];
						}
					}
				}
				updateGrid();
			...
			clear_hurt_notes.color = FlxColor.RED;
			clear_hurt_notes.label.color = FlxColor.WHITE;
		...
		tab_group_song.add(clear_hurt_notes);
